...
def generate_density_shards(opt):
    ...
    images_dir = Path(opt["base_dir"]) / "data" / \
        opt["scene"] / f'{opt["resolution"]}x'
    image_filenames = {idx: val for idx,
                       val in enumerate(sorted(os.listdir(images_dir)))}
    ...
    print(
        f"Finished density sharding with {len(shard_id2image_filenames)} shards.")
...
def generate_radial_shards(opt, waypoints):
    ...
    images_dir = Path(opt["base_dir"]) / "data" / \
        opt["scene"] / f'{opt["resolution"]}x'
    image_filenames = {idx: val for idx,
                       val in enumerate(sorted(images_dir.iterdir()))}
    ...
    for waypoint_id in range(waypoints.shape[0]):
        ...
        if (shard_id in raw_shard_id2image_filenames):
            raw_shard_id2image_filenames[shard_id].append(
                image_filenames[waypoint_id])
        else:
            raw_shard_id2image_filenames[shard_id] = [
                image_filenames[waypoint_id]]
    ...
    print(
        f"Finished radial sharding with {len(shard_id2image_filenames)} shards.")
    return shard_id2image_filenames
...
def init_shard_dir(opt, shard_dirname, image_filenames):
    shard_dir = Path(opt["base_dir"]) / "exp" / \
        (opt["scene"]+f"_{opt['resolution']}x") / shard_dirname
    src_images_dir = Path(opt["base_dir"]) / "data" / \
        opt["scene"] / f'{opt["resolution"]}x'
...
def load_waypoints(opt):
    waypoints_fp = Path(opt["base_dir"]) / "data" / \
        opt["scene"] / "waypoints.npy"
...
def init_colmap_shard(opt, shard_dir, shard_id, waypoints, shard_id2image_filenames):
    ...
    if (n_waypoints < 5):
        print(
            f" Skipping Shard {shard_id} because it only has {n_waypoints} waypoints")
        return
    print(f"Shard {shard_id} has {n_waypoints} waypoints")

    waypoints_idxs = [int(os.path.splitext(image_filename)[0])
                      for image_filename in image_filenames]
    curr_shard_waypoints = {idx: waypoints[idx] for idx in waypoints_idxs}
    translate, scale = get_tf_cams(
        curr_shard_waypoints, target_radius=opt["target_radius"])
    ...
    with open(models_dir / "images.txt", 'w') as fout:
        ...
        fout.write(
            f"# Number of images: {n_waypoints}, mean observations per image: X\n")
        ...
        for i, img_name in enumerate(sorted(os.listdir(images_dir))):
            ...
            fout.write(
                f"{i+1} {qw} {qx} {qy} {qz} {x} {y} {z} 1 {img_name}\n \n")
...
def generate_shards(opt):
    ...
    if (opt["sharding"] == "radial"):
        ...
        for shard_id in shard_id2image_filenames.keys():
            shard_dir = init_shard_dir(
                opt, f"shard{shard_id+1}_of_{n_shards}", shard_id2image_filenames[shard_id])
            if (opt["run_sensor_colmap"]):
                init_colmap_shard(opt, shard_dir, shard_id,
                                  corrected_waypoints, shard_id2image_filenames)
...
def rename(opt):
    raw_images_dir = Path(opt["base_dir"]) / "data" / \
        (opt["scene"]) / "raw_images"
...
def downsample(opt):
    ...
    if opt["resolution"] == 1:
        ...
        for image_path in sorted(Path(images_path).glob('*.jpg')):
            ...
            save_image(
                dst,
                image_to_uint8(downsample_image(image, opt["resolution"])))
...
#     image_filenames = sorted((data_dir / "images").iterdir())
    ...
    images_paths = {idx: val for idx, val in enumerate(
        sorted((data_dir / "images").iterdir()))}
#     print(list(images_paths.values()))
    ...
    print(
        f'Filtering {len(blur_filter_paths)} IDs: {[path.name for path in blur_filter_paths]}')
    ...
    plt.subplot(121)
    plt.title(
        f'Least blurry among filtered: {images_paths[blur_filter_inds[-1]].name}')
    plt.imshow(images[blur_filter_inds[-1]])
    plt.subplot(122)
    plt.title(
        f'Most blurry among filtered: {images_paths[blur_filter_inds[0]].name}')
    plt.imshow(images[blur_filter_inds[0]])


def color_correct(opt):
    calibration_img_path = Path(
        opt["base_dir"])/"data" / opt["scene"] / opt["calibration_relative_path"]
    # calibration_img_path = Path(opt["base_dir"])/"exp"/(
    #     opt["scene"]+f"_{opt['resolution']}x") / opt["calibration_relative_path"]
    calibration_image = colour.cctf_decoding(
        colour.io.read_image(calibration_img_path))

    colour.plotting.plot_image(colour.cctf_encoding(calibration_image))
    seg_results = detect_colour_checkers_segmentation(
...
#     # for more information.
    ...
    swatches_xyY = colour.XYZ_to_xyY(colour.RGB_to_XYZ(
        swatch_colours, D65, D65, colour.RGB_COLOURSPACES['sRGB'].matrix_RGB_to_XYZ))

    colour_checker = colour.characterisation.ColourChecker(
        os.path.basename(calibration_img_path),
    ...
    swatches_f = colour.colour_correction(
        swatch_colours, swatch_colours, REFERENCE_SWATCHES)
    ...
    colour.plotting.plot_image(colour.cctf_encoding(
        colour.colour_correction(
            calibration_image, swatch_colours, REFERENCE_SWATCHES)))
    ...
    image_filenames = sorted([f for f in image_dir.iterdir(
    ...
    for image_filename in image_filenames:
        ...
        corrected_image = colour.colour_correction(
            image, swatch_colours, REFERENCE_SWATCHES)
    ...
    colour_checkers, clusters, swatches, segmented_image = (
        colour_checkers_coordinates_segmentation(
            calibration_image, additional_data=True).values)
...
def get_tf_cams(waypoints, target_radius=1.):
